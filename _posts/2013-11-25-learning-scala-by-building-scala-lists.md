...
This includes plugins to generate project files for both IDEs. If you're using Eclipse, just type `sbt eclipse` and import the project, if you're on IntelliJ, just use `sbt gem-idea` and import the project. If you're not using any of them, use `sbt build` and `sbt test` to compile and run tests for your project.

## Lists, camera, action!
Lists are one of the basic constructs of funcional programming and it couldn't be different in Scala. Let's start by defining our LinkedList object:
...
Moving now, now we have `Node[+E]`. First, what's a `case class`? A case class is a class that allows you easily pattern match on them using it's constructor. You could manually do it, but by defining it as a case class you can forget about all that and just use all the pattern matching magic we will see below.
...
As you can see, it doesn't hold any values and it's type parameter is `Nothing`. In Scala, `Nothing` is the bottom type of all types. It's meant to be used in these special cases of generics when you want to sinal the lack of a value but still be type safe. But don't worry too much about it, generics won't affect us much here.
...
In Scala, when you do `something()`, you're actually calling the `apply()` method. It's just syntax sugar around the syntax. And as we have seen before, since Scala doesn't have static methods, we declare a singleton object called `LinkedList` (given there is a trait named `LinkedList`, we say this is the companion object for `LinkedList`) and now we can declare any methods that would not make sense at the `LinkedList` itself here.
...
What is the goal of the `map` method? To transform a data structure by applying a definite operation on every element of the structure and returning a structure with the result. So, if we had the following list `(2,3,4)` and the map operation was squaring them it would produce the list `(4,9,16)`.
...
Can you see the pattern here? We have a default value and we take the current item and joint it with the recursive call with the tail of our list. These are classic examples of a `fold` operation.
...
Now on to the implementation itself, there isn't much here. It's a common recursive function where we compute the next accumulator and send it forward with a recursive call. If we have reached the end of the list, just return the current accumulator itself.
...
And we have covered lists, map and foldLeft here, all very important constructs when doing funcional programming and working with Scala in general. Next we will move on to a couple other methods on lists and more funcional programming constructs. Stay tuned!
...
* [Funcional Programming Principles in Scala course by Martin Odersky](https://www.coursera.org/course/progfun)
* [Funcional Programming in Scala book by Paul Chiusiano and Rúnar Bjarnson](http://www.manning.com/bjarnason/)